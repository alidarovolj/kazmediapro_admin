  ...
  <div>
    <div class="mb-3 lg:mb-5 lg:w-full">
      ...
      <p class="text-sm text-center">
        Вы уверены что хотите удалить данный кейс? Вы больше не сможете
...
export default {
  ...
  methods: {
    ...mapActions(["cases", "removeCase"]),
    async removeCaseLocal() {
      this.loading = true;

      await this.removeCase(this.form);
      if (this.getRemovedCase) {
        this.loading = false;
        this.cases();
        this.notify(true, "Кейс удален");
        this.closeModal();
      }
    },
    closeModal() {
  ...
  computed: {
    ...mapGetters(["getCases", "getRemovedCase"]),
  },
  setup() {
